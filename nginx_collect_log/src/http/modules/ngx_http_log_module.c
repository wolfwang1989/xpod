...
static rd_kafka_t* produce = NULL;
...
#endif
	if(!conf){
		init(log->format->brokerList.data);
	}
	if(!log->format->topic){
		log->format->topic = createProduceTopic(log->format->topicName.data);
	}
	rd_kafka_produce_random_copy_no_key(log->format->topic,buf,len);
    if (log->script == NULL) {
...
ngx_http_log_set_format(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    fmt->name = value[1];
	fmt->topicName = value[2];
	fmt->brokerList = value[3];
    ...
    return ngx_http_log_compile_format(cf, fmt->flushes, fmt->ops, cf->args, 4);
}
