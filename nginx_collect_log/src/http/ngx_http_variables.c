    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ...
    if (cl->next == NULL) {
	if((buf->last - buf->pos) > 3)
	{
		if(ngx_strncmp(buf->pos,"{\"data\":",3) != 0)
		{
			ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"one buf len %z",(buf->last - buf->pos));
			de_data = ngx_palloc(r->pool,TTPOD_MAX_PER_LOG_LEN);
			if(de_data == NULL)
			{
				ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"one request_body alloc failed len:%z",(buf->last - buf->pos));
				return NGX_ERROR;
			}
			if(httpgzdecompress((Byte*)buf->pos, (buf->last - buf->pos),(Byte*)de_data, &outlen) != 0) 
			{
				ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"one request_body decompress failed:%z",(buf->last - buf->pos));
				return NGX_ERROR;	
			}
				ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"one request_body decompress success:%z",(buf->last - buf->pos));
				ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"one request_body outlen:%uL",outlen);
			v->len = outlen;
			v->valid = 1;
			v->no_cacheable = 0;
			v->not_found = 0;
    ...
    if(len > 3)
        {
                if(ngx_strncmp(p-len,"{\"data\":",3) != 0)
                {
			ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"many buf len %z",len);
                        de_data = ngx_palloc(r->pool,TTPOD_MAX_PER_LOG_LEN);
                        if(de_data == NULL)
                        {
				ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"request_body alloc failed len:%i",len);
                                return NGX_ERROR;
                        }
			if(iRet = httpgzdecompress((Byte*)(p-len), len,(Byte*)de_data, &outlen) != 0)
			{
				ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"request_body decompress failed:%i",iRet);
				return NGX_ERROR; 
			}
			ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"request_body decompress success de_len:%uL",outlen);
                        v->len = outlen;
                        v->valid = 1;
                        v->no_cacheable = 0;
                        v->not_found = 0;
